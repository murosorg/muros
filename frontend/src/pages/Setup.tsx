...
// faces the Internet (WAN) and which faces the trusted LAN. Applying it
// assigns the zones, drops the permissive bootstrap rules and pushes the
// network + firewall configuration. After that the box reaches its final
// posture: LAN can reach everything, the WAN is default-deny.
export default function Setup() {
  ...
  useEffect(() => {
    api.setup.state().then((s) => {
      ...
      if (names[0]) setWan(names[0])
      if (names[1]) setLan(names[1])
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.setup.apply({ wan_interface: wan, lan_interface: lan, lan_cidr: lanCidr })
    ...
    <div className="min-h-full flex items-center justify-center bg-gray-50 py-10">
      <form onSubmit={submit} className="w-full max-w-lg bg-white border border-gray-200 rounded-md p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-600 mb-5">
          Assign your network interfaces. The LAN is the trusted side and
          can reach the firewall and its services; the WAN (Internet) is
          blocked inbound by default.
        </p>

        {error && <div className="mb-4"><ErrorBlock message={error} /></div>}

        <div className="mb-4">
          <label className="label">WAN interface (Internet, DHCP client)</label>
        ...
        <button type="submit" className="btn-primary w-full justify-center"
          disabled={busy || !wan || !lan || wan === lan || !lanCidr}>
