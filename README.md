...
- **Drop-ins over file rewrites.** When a daemon supports drop-ins (sshd,
  fail2ban, snmpd, swanctl, dnsmasq, unbound...) MurOS uses them. Your
...
·  `dnsmasq + unbound`  ·  `fail2ban + snmpd`
...
| DHCP server | dnsmasq backend, per-interface pools, static leases by MAC, active lease view |
...
/etc/dnsmasq.d/muros-dhcp.conf             dnsmasq drop-in (DHCP only)
...
- **V1.0** (beta, current rc cycle): Stateful filtering, NAT, routing, VLAN,
  ...
  hardening, apt updates, multi-WAN failover, built-in DHCP (dnsmasq) and
