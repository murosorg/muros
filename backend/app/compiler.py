...
def _format_addr_set(values: list[str]) -> str:
    """Format a list of IP/CIDR values as an inline nft set."""
    if len(values) == 1:
        return values[0]
    return "{ " + ", ".join(values) + " }"
...
def _compile_proto_ports(rule: models.FirewallRule) -> list[str]:
    """Return the proto+ports portion(s) of a rule.

    When rule.service_group is set, return one variant per protocol
    present in the group (nft cannot mix tcp/udp in a single dport set).
    Otherwise, classic behavior using the protocol/dst_port/src_port
    fields.
    """
    ...
    # Protocol "any" (or missing): if ports are set, emit one line per
    # port-capable protocol (tcp + udp). Otherwise, no selector (the rule
    ...
    if proto in ("tcp", "udp"):
        ...
        if not sport and not dport:
            # Match the transport protocol regardless of IP family. The
            # filter table is `inet`, so `ip protocol tcp` would only
            # match IPv4 and silently skip IPv6; `meta l4proto` covers
            # both, mirroring the ICMP handling above.
            bits.append(f"meta l4proto {proto}")
        return [" ".join(bits)]
    return [""]
...
def compile_ruleset(db: Session) -> str:
    ...
    for r in rules:
        # _compile_rule may return several lines (multi-protocol
        # service_group case).
    ...
    out.append("# Generated by MurOS - do not edit manually")
