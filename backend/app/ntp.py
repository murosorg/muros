...
- MurOS drops /etc/chrony/conf.d/muros.conf with the ``server`` lines
  ...
  chrony configuration stays intact.
...
def _write_conf(servers: list[str], serve_lan: bool = True) -> None:
...
def set_servers(servers: list[str], serve_lan: bool = True) -> None:
...
def apply_config(db, servers: list[str] | None = None) -> None:
    """Reconcile the chrony drop-in from the DB config.

    Writes the upstream servers plus ``allow all`` when server mode is
    enabled (NtpConfig.serve_lan), then restarts chrony. Used by the API
