...
def render_swanctl_conf(connections: list, certs_by_id: dict | None = None) -> str:
    ...
    for c in connections:
        ...
        lines.extend([
            ...
            # Dead Peer Detection: probe IKE_SA liveness every 30s. Without
            # dpd_delay (default 0) strongSwan never probes, so the child
            # dpd_action below would never fire and a dead tunnel would hang
            # until the next rekey. 30s gives fast failover on site-to-site.
            "        dpd_delay = 30s",
            *local_lines,
            *remote_lines,
            "        children {",
...
def render_swanctl_secrets(connections: list, certs_by_id: dict | None = None) -> str:
    ...
    - PSK mode: ike-<name> { secret = "..." id-1 = ... id-2 = ... }
    - cert mode: private-<name> { file = muros-<cert>-key.pem }
    """
    if certs_by_id is None:
        certs_by_id = {}
