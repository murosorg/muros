...
def quick_create_peer(db, name: str, description: str | None = None):
    """One-shot peer creation with all the defaults a road-warrior needs.

    Generates the peer keypair and PSK, allocates the next free /32 in
    the tunnel subnet, and sets PersistentKeepalive so the peer punches
    through NAT without further tweaking. Returns (peer, private_key).
    The peer private key is returned ONCE and never stored server-side.
    """
    ...
    if cfg is None:
        ...
        db.add(cfg)
        db.flush()
    if ensure_initialized(cfg):
        db.flush()

    peers = db.query(models.WireGuardPeer).all()
    if any(p.name == name for p in peers):
        raise ValueError(f"A peer named '{name}' already exists.")

    priv, pub = _curve25519_keys()
    psk = generate_psk()
    allowed = next_free_peer_ip(cfg, peers)
    ...
    peer = models.WireGuardPeer(
        ...
        preshared_key=psk,
        allowed_ips=allowed,
        # Sensible default for road-warriors : full tunnel. Admin can
        # narrow it down to a split tunnel from the peer edit modal.
